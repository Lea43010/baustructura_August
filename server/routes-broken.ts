...
export async function registerRoutes(app: Express): Promise<Server> {
      ...
      try {
        ...
        try {
          ...
          const fallbackResult = {
            ...
            size: fileBuffer.length,
            uploadedAt: new Date(),
            message: "Datei temporär lokal gespeichert (SFTP-Server nicht verfügbar)",
            storage: 'local',
            attachment
          };
  ...
  // SFTP legacy routes cleaned up - all functionality migrated to Supabase Storage
  ...
  app.post("/api/projects", isAuthenticated, async (req: SecurityRequest, res) => {
    try {
      ...
      // LIZENZ-LIMITIERUNG: Prüfe Projekt-Limits basierend auf Lizenz-Typ
      const licenseFeatures = getLicenseFeatures(user.licenseType || 'basic');
      console.log(`📊 LICENSE CHECK: UserID: ${userId}, License: ${user.licenseType}, Max Projects: ${licenseFeatures.maxProjects}`);
      
      if (licenseFeatures.maxProjects !== -1) {
        const userProjects = await storage.getProjects(userId);
        if (userProjects.length >= licenseFeatures.maxProjects) {
          console.log(`🚫 PROJECT LIMIT REACHED: UserID: ${userId}, Current: ${userProjects.length}, Limit: ${licenseFeatures.maxProjects}`);
          return res.status(403).json({
            error: 'Project limit reached',
            message: `Ihr ${user.licenseType || 'Basic'} Plan erlaubt maximal ${licenseFeatures.maxProjects} Projekte. Upgraden Sie auf Professional für unbegrenzte Projekte.`,
  ...
  app.put("/api/projects/:id", isAuthenticated, async (req: any, res) => {
    try {
      ...
      console.log(`🔧 PROJECT UPDATE: UserID: ${userId}, ProjectID: ${projectId}, Role: ${user?.role}`);
  ...
  app.post("/api/customers", isAuthenticated, async (req: any, res) => {
    try {
      ...
      // LIZENZ-LIMITIERUNG: Prüfe Kunden-Limits basierend auf Lizenz-Typ
      const licenseFeatures = getLicenseFeatures(user.licenseType || 'basic');
      console.log(`📊 LICENSE CHECK: UserID: ${userId}, License: ${user.licenseType}, Max Customers: ${licenseFeatures.maxCustomers}`);
      ...
      if (licenseFeatures.maxCustomers !== -1) {
        ...
        if (filteredCustomers.length >= licenseFeatures.maxCustomers) {
          console.log(`🚫 CUSTOMER LIMIT REACHED: UserID: ${userId}, Current: ${filteredCustomers.length}, Limit: ${licenseFeatures.maxCustomers}`);
          return res.status(403).json({
            error: 'Customer limit reached',
            message: `Ihr ${user.licenseType || 'Basic'} Plan erlaubt maximal ${licenseFeatures.maxCustomers} Kunden. Upgraden Sie auf Professional für mehr Kunden.`,
  ...
  app.get("/api/flood/checklists", isAuthenticated, async (req: any, res) => {
    try {
      // Echte Datenbank-Abfrage für Hochwasserschutz-Checklisten
  ...
  app.post("/api/flood/create-checklist", isAuthenticated, async (req: any, res) => {
    try {
      ...
      // LIZENZ-LIMITIERUNG: Hochwasserschutz nur für Professional und Enterprise
      const licenseFeatures = getLicenseFeatures(user?.licenseType || 'basic');
      if (!licenseFeatures.allowsHochwasserschutz) {
        console.log(`🚫 HOCHWASSERSCHUTZ BLOCKED: UserID: ${userId}, License: ${user?.licenseType || 'basic'}`);
        return res.status(403).json({
  ...
  app.get("/api/flood/absperrschieber", isAuthenticated, async (req: any, res) => {
    try {
      // Echte Datenbank-Abfrage für Absperrschieber
      ...
      res.json({
        ...
        message: schieber.length === 0 ? "Keine Absperrschieber vorhanden. Fügen Sie den ersten hinzu." : undefined
      });
    } catch (error) {
  ...
  // 📧 E-MAIL-WEITERLEITUNG: BREVO Inbound Webhook
  app.post('/api/inbound-email', async (req, res) => {
    await emailForwardingService.handleInboundEmail(req, res);
  });

  // 🔧 E-Mail-Weiterleitungsregeln verwalten (Admin)
  ...
  // 📋 Setup-Anweisungen für BREVO Inbound API abrufen
...
Diese E-Mail wurde automatisch generiert vom Bau-Structura Hochwasserschutz-System.
  ...
  app.put("/api/admin/users/:id/role", isAuthenticated, isAdmin, async (req: any, res) => {
    try {
        ...
        sessionStore.all((err: any, sessions: any) => {
          ...
          Object.keys(sessions || {}).forEach(sessionId => {
            ...
            if (session?.passport?.user === user.id) {
              console.log(`🔄 Updating session for user ${user.email} with new role: ${role}`);
      ...
      res.json({ 
        success: true, 
        message: `Rolle von ${user.email} zu ${role} geändert. Die Änderung ist sofort aktiv.`,
        user: { 
          id: user.id, 
          email: user.email, 
          role: role,
  ...
  app.post('/api/ai/generate-description', isAuthenticated, async (req: any, res) => {
    try {
      ...
      // LIZENZ-LIMITIERUNG: KI-Features nur für Enterprise
      const licenseFeatures = getLicenseFeatures(user?.licenseType || 'basic');
      if (!licenseFeatures.allowsAI) {
        console.log(`🚫 KI-BESCHREIBUNG BLOCKED: UserID: ${userId}, License: ${user?.licenseType || 'basic'}`);
        return res.status(403).json({
  ...
  app.post('/api/ai/risk-assessment', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const licenseFeatures = getLicenseFeatures(user?.licenseType || 'basic');
      if (!licenseFeatures.allowsAI) {
        console.log(`🚫 KI-RISIKOBEWERTUNG BLOCKED: UserID: ${userId}, License: ${user?.licenseType || 'basic'}`);
        return res.status(403).json({
          error: 'Premium Feature',
  ...
  app.post('/api/ai/project-chat', isAuthenticated, async (req: any, res) => {
    try {
      ...
      const licenseFeatures = getLicenseFeatures(user?.licenseType || 'basic');
      if (!licenseFeatures.allowsAI) {
        console.log(`🚫 KI-CHAT BLOCKED: UserID: ${userId}, License: ${user?.licenseType || 'basic'}`);
        return res.status(403).json({
          error: 'Premium Feature',
  ...
  app.post('/api/audio/transcribe', isAuthenticated, async (req: any, res) => {
    try {
      ...
      // ⚠️ SIMULATION: In Produktionsumgebung wird OpenAI Whisper API verwendet
      const simulationTranscriptions = [
        "[SIMULATION] Die Bauarbeiten am Fundament sind heute planmäßig vorangeschritten. Die Betonierung wurde erfolgreich abgeschlossen.",
        "[SIMULATION] Heute wurde die Bewehrung für die Bodenplatte eingebaut. Qualitätsprüfung durch den Statiker erfolgt morgen.",
        "[SIMULATION] Fortschritt bei der Rohrleitungsverlegung. Alle Anschlüsse sind fachgerecht ausgeführt worden.",
        "[SIMULATION] Die Erdarbeiten sind abgeschlossen. Nächster Schritt ist die Verdichtung des Untergrunds.",
        "[SIMULATION] Schalung für die Stützen wurde heute aufgestellt. Betonage ist für morgen geplant."
      ];
      ...
      res.json({ 
        ...
        language: "de",
        isSimulation: true,
        message: "⚠️ Dies ist eine simulierte Transkription. Für echte Spracherkennung wird OpenAI Whisper API benötigt."
      });
    } catch (error) {
  ...
  // ✅ DUALES SUBSCRIPTION-SYSTEM: Monthly vs Annual
  app.post("/api/create-payment-intent", isAuthenticated, async (req: any, res) => {
    try {
      ...
      if (subscriptionType === 'annual') {
        ...
        // 📅 MONATLICHE ZAHLUNG = VOLLER PREIS
        finalPrice = monthlyPrice;
        subscriptionInterval = 1;
        description = `${licenseType} Lizenz (monatlich kündbar)`;
      }
      
      console.log(`💰 Pricing: ${licenseType} - ${subscriptionType}`);
      console.log(`   Monthly: ${monthlyPrice}€`);
      console.log(`   Annual: ${monthlyPrice * 12 * 0.9}€ (10% Rabatt)`);
  ...
  // ✅ VOLLSTÄNDIGER STRIPE WEBHOOK HANDLER MIT AUTOMATISCHER LIZENZ-AKTIVIERUNG UND E-MAIL-BESTÄTIGUNG
    ...
    try {
      ...
      switch (event.type) {
        case 'payment_intent.succeeded':
          ...
          if (!userId) {
            console.error('❌ Keine userId in PaymentIntent Metadaten gefunden');
            break;
          }
          ...
          try {
            ...
            await storage.updateUser(userId, updateData);
            console.log(`✅ Lizenz automatisch aktiviert für User ${userId}: ${licenseType}`);
            
            // 2. E-MAIL-BESTÄTIGUNG AN KUNDEN SENDEN
            if (userEmail) {
              const { emailService } = await import('./emailService');
              ...
              console.log(`📧 Zahlungsbestätigungs-E-Mail gesendet an: ${userEmail}`);
            }
            ...
            try {
              ...
              const sftpResult = await onLicenseActivated(userId, licenseType);
              console.log(`🔧 SFTP-Setup:`, sftpResult.success ? '✅ Erfolgreich' : '⚠️ Übersprungen');
            } catch (sftpError) {
              console.log('⚠️ SFTP-Setup übersprungen (nicht kritisch):', sftpError);
            }
            
          } catch (error) {
            console.error('❌ Fehler bei Lizenz-Aktivierung:', error);
            // Webhook sollte trotzdem 200 zurückgeben, um Retry-Schleifen zu vermeiden
          }
  ...
  app.post('/api/admin/trial-notifications/check', isAuthenticated, requireAdmin, async (req: any, res) => {
    try {
      ...
      console.error('❌ Error running trial notification check:', error);
  ...
  app.get('/api/user/trial-notifications', isAuthenticated, async (req: any, res) => {
    try {
      ...
      console.error('❌ Error fetching user trial notifications:', error);
