...
export async function setupLocalAuth(app: Express) {
  ...
  passport.serializeUser((user: any, done) => {
    console.log('🔍 Serializing user:', user.externalId, user.email);
    done(null, user.externalId); // Verwende externalId für Session-Kompatibilität
  });
  ...
  passport.deserializeUser(async (id: string, done) => {
    try {
      ...
      // Automatische Lizenz-Status-Überprüfung
      const now = new Date();
      let needsUpdate = false;
      let updatedUser = { ...user };

      // Prüfe Trial-Ablauf und setze licenseStatus entsprechend
      if (user.trialEndDate && user.trialEndDate < now) {
        if (user.licenseStatus === 'active') {
          console.log('⏰ Trial expired for user:', user.email, '- Setting license to inactive');
          updatedUser.licenseStatus = 'inactive';
          needsUpdate = true;
        }
      } else if (user.trialEndDate && user.trialEndDate >= now) {
        if (user.licenseStatus === 'inactive') {
          console.log('✅ Trial still active for user:', user.email, '- Setting license to active');
          updatedUser.licenseStatus = 'active';
          needsUpdate = true;
        }
      }

      // Aktualisiere Datenbank wenn Status geändert wurde
      if (needsUpdate) {
        try {
          await storage.updateUser(id, { licenseStatus: updatedUser.licenseStatus as any });
          console.log('💾 License status updated for user:', user.email, 'to:', updatedUser.licenseStatus);
        } catch (updateError) {
          console.error('❌ Failed to update license status:', updateError);
          // Verwende trotzdem den aktualisierten Status für diese Session
        }
      }

      console.log('✅ User successfully deserialized:', user.email, 'License Status:', updatedUser.licenseStatus);
  ...
  app.post("/api/auth/register", async (req, res) => {
    try {
      ...
      try {
        ...
        console.log(`✅ SFTP-Account automatisch erstellt für Benutzer ${user.externalId}: ${sftpUsername}`);
      } catch (sftpError) {
